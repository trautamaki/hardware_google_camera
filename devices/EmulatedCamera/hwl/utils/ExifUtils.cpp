...
public:
    ExifUtilsImpl(SensorCharacteristics sensorChars);
...
ExifUtils *ExifUtils::create(SensorCharacteristics sensorChars) {
    return new ExifUtilsImpl(sensorChars);
}

ExifUtils::~ExifUtils() {
}

ExifUtilsImpl::ExifUtilsImpl(SensorCharacteristics sensorChars)
        : exif_data_(nullptr), app1_buffer_(nullptr), app1_length_(0), mSensorChars(sensorChars) {}
...
bool ExifUtilsImpl::setFromMetadata(const HalCameraMetadata& metadata, size_t imageWidth,
        size_t imageHeight) {
    ...
    if (ret == OK) {
        ...
        if (!setFocalLengthIn35mmFilm(
                    focal_length, mSensorChars.physicalSize[0], mSensorChars.physicalSize[1])) {
    ...
    if (ret == OK) {
        if (!setDigitalZoomRatio(entry.data.i32[2], entry.data.i32[3], mSensorChars.width,
                    mSensorChars.height)) {
    ...
    if (ret == OK) {
        ...
        metadata.Get(ANDROID_CONTROL_POST_RAW_SENSITIVITY_BOOST, &postRawSensEntry);
    ...
    camera_metadata_ro_entry flashStateEntry;
    metadata.Get(ANDROID_FLASH_STATE, &flashStateEntry);
    camera_metadata_ro_entry aeModeEntry;
    metadata.Get(ANDROID_CONTROL_AE_MODE, &aeModeEntry);
    ...
    if (!setFlash(mSensorChars.isFlashSupported, flashState, aeMode)) {
